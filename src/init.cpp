...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: darksaga.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: darksaga.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizedarksagaamount=<n> " + _("Keep N DarkSaga anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DarkSaga is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DarkSaga is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DarkSaga version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "DarkSaga server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DarkSaga") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DarkSaga to complete") << "\n";
    ...
    if(fMasterNode) {
        LogPrintf("IS SAGASEND MASTER NODE\n");
    ...
    nAnonymizeDarkSagaAmount = GetArg("-anonymizedarksagaamount", 0);
    if(nAnonymizeDarkSagaAmount > 999999) nAnonymizeDarkSagaAmount = 999999;
    if(nAnonymizeDarkSagaAmount < 2) nAnonymizeDarkSagaAmount = 2;
    ...
    LogPrintf("Anonymize DarkSaga Amount %d\n", nAnonymizeDarkSagaAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1SAGA+1000 == (.1SAGA+100)*10
       10SAGA+10000 == (1SAGA+1000)*10
    */
