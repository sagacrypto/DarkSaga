...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDarkSagaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDarkSagaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDarkSagaAmount = strAnonymizeDarkSagaAmount.remove(strAnonymizeDarkSagaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDarkSagaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDarkSagaAmount*COIN) nMaxToAnonymize = nAnonymizeDarkSagaAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDarkSagaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDarkSagaAmount));
        strAnonymizeDarkSagaAmount = strAnonymizeDarkSagaAmount.remove(strAnonymizeDarkSagaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDarkSagaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
