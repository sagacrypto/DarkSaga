...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("DarkSaga"))
        return false;
...
{
    // Convert darksaga:// to darksaga:
    //
    //    Cannot handle this later, because darksaga:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("darksaga://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "darksaga:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DarkSaga.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DarkSaga.lnk
...
{
    return GetAutostartDir() / "DarkSaga.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a DarkSaga.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DarkSaga\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the DarkSaga app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add DarkSaga app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DarkSaga-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DarkSaga-qt [" + tr("command-line options") + "]                     " + "\n";
