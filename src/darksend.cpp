...
void CDarksendPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        LogPrintf("CDarksendPool::Check() -- IS MASTER -- TRANSMITTING SAGASEND\n");
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat DarkSaga and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002SAGA per transaction on average.
            */
            if(r <= 10)
            {
...
void CDarksendPool::CheckTimeout(){
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE){
        ...
        if(GetTimeMillis()-lastTimeChanged >= (SAGASEND_QUEUE_TIMEOUT*1000)+addLagTime){
            UnlockCoins();
            SetNull();
        }
    } else if(GetTimeMillis()-lastTimeChanged >= (SAGASEND_QUEUE_TIMEOUT*1000)+addLagTime){
        LogPrint("darksend", "CDarksendPool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", SAGASEND_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if(state == POOL_STATUS_SIGNING && GetTimeMillis()-lastTimeChanged >= (SAGASEND_SIGNING_TIMEOUT*1000)+addLagTime ) {
            LogPrint("darksend", "CDarksendPool::CheckTimeout() -- Session timed out (%ds) -- restting\n", SAGASEND_SIGNING_TIMEOUT);
            ChargeFees();
            UnlockCoins();
            SetNull();
...
bool CDarksendPool::IsCollateralValid(const CTransaction& txCollateral){
    ...
    //collateral transactions are required to pay out SAGASEND_COLLATERAL as a fee to the miners
    if(nValueIn-nValueOut < SAGASEND_COLLATERAL) {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    // should not be less than fees in SAGASEND_COLLATERAL + few (lets say 5) smallest denoms
    CAmount nLowestDenom = SAGASEND_COLLATERAL + darkSendDenominations[darkSendDenominations.size() - 1]*5;

    // if there are no DS collateral inputs yet
    if(!pwalletMain->HasCollateralInputs())
        // should have some additional amount for them
        nLowestDenom += SAGASEND_COLLATERAL*4;

    CAmount nBalanceNeedsAnonymized = nAnonymizeDarkSagaAmount*COIN - pwalletMain->GetAnonymizedBalance();

    // if balanceNeedsAnonymized is more than pool max, take the pool max
    if(nBalanceNeedsAnonymized > SAGASEND_POOL_MAX) nBalanceNeedsAnonymized = SAGASEND_POOL_MAX;
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    vecSend.push_back(make_pair(scriptCollateral, SAGASEND_COLLATERAL*4));
...
bool CDarksendPool::CreateDenominated(int64_t nTotalValue)
{
    ...
    if(!pwalletMain->HasCollateralInputs()) {
        vecSend.push_back(make_pair(scriptCollateral, SAGASEND_COLLATERAL*4));
        nValueLeft -= SAGASEND_COLLATERAL*4;
    }
    ...
    BOOST_REVERSE_FOREACH(int64_t v, darkSendDenominations){
        ...
        while(nValueLeft - v >= SAGASEND_COLLATERAL && nOutputs <= 10) {
            CScript scriptDenom;
            CPubKey vchPubKey;
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100SAGA+1 ( bit on if present )
    // bit 1 - 10SAGA+1
    // bit 2 - 1SAGA+1
    // bit 3 - .1SAGA+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("darksaga-darksend");
