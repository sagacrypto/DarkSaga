...
int nAnonymizeDarkSagaAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DarkSaga
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DarkSaga
    // Mac: ~/Library/Application Support/DarkSaga
    // Unix: ~/.DarkSaga
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DarkSaga";
#else
    fs::path pathRet;
...
static void WriteConfigFile(FILE* configFile)    //append darksaga.conf
{
    fputs ("litemode=1\n", configFile);
    fclose(configFile);
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "darksaga.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty darksaga.conf if it doesn't exist
    ...
    {
        // Don't overwrite existing settings so command line settings override darksaga.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "darksaga.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong DarkSaga will not work properly.");
