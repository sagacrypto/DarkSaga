...
const string strMessageMagic = "DarkSaga Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight <= 1) {
        nSubsidy = 2100000 * COIN; //pre-mine
    }
  ...
  else if (nHeight < 944600){
    nSubsidy = (25 / 4) * COIN;
    }
  else if (nHeight < 944601){
    nSubsidy = 3353300 * COIN; //post-mine
    }
  else if (nHeight < 1043122){
    nSubsidy = 2 * COIN;
    }
    else {
        nSubsidy = 1 * COIN;
...
/*string getDevAddress(int nHeight) {
	...
	if (addrId == 0) {
		return "sWxwp7bnJNXiuBAkDBLqLmbKRWrn5URQ7S";
	} else if (addrId == 1) {
		return "sWxwp7bnJNXiuBAkDBLqLmbKRWrn5URQ7S";
	} else if (addrId == 2) {
		return "sWxwp7bnJNXiuBAkDBLqLmbKRWrn5URQ7S";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight < YEAR1_BLOCKS)	{
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
    }
    else if (pindexBest->nHeight < YEAR2_BLOCKS)	{
      nSubsidy = nCoinAge * (COIN_YEAR_REWARD / 2) * 33 / (365 * 33 + 8);;
    }
    else if (pindexBest->nHeight < 944600)	{
      nSubsidy = nCoinAge * (COIN_YEAR_REWARD / 4) * 33 / (365 * 33 + 8);;
    }
    else if (pindexBest->nHeight < 1043122)	{
      nSubsidy = 4 * COIN;
    }
    else {
      nSubsidy = 2 * COIN;
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    if (false) {
                        ...
                        CDarkSagaCoinAddress devRewardAddress(getDevAddress(pindex->nHeight + 1));
